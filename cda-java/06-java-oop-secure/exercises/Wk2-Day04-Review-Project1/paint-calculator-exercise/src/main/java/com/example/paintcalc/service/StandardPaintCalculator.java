...
public final class StandardPaintCalculator implements PaintCalculator {
    public PaintEstimate calculate(long userId, Room room, int coats, PaintColor color, PaintProduct product) {
        ...
        BigDecimal totalArea = room.wallSquareFeet().multiply(BigDecimal.valueOf(coats));
        int gallons = totalArea.divide(product.coveragePerGallon(), 0, RoundingMode.UP).intValue();
        BigDecimal cost = product.pricePerGallon().multiply(BigDecimal.valueOf(gallons));
        return new PaintEstimate(userId, room.wallSquareFeet(), coats, color, gallons, cost);
    }
}
